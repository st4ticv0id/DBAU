            ...
            if (errorGeneric.encode() in stdout):
                ...
                try:
                    ...
                    if (os.path.isfile(f"{mainPath}/logs/genericError_{currentDate}.log") == False):
                        ...
                        DNFEFile.write(f"BEGIN ERROR => {genericErrorString} == END ERROR (issue report: INSERISCI QUI)")
                        DNFEFile.close()

                    print("")
                    print(Back.RED + f"ERROR: Directory '{mainPath}/logs' could not be found. Please report your problem on INSERISCI QUI" + defaultBackColor) #Sostituire INSERISCI QUI con il link della pagina issue del progetto github
